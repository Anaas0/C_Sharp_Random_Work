...
namespace PredatorPrey
{
    ...
    class Location//11
    {
        ...
        public char TypeOfTerrain;
        public Location(char inTypeOfTerrain)
        {
            Fox = null;
            Warren = null;
            TypeOfTerrain = inTypeOfTerrain;
        }
    }
    ...
    class Simulation//11
    {
        ...
        private void CreateLandscapeAndAnimals(int InitialWarrenCount, int InitialFoxCount, bool FixedInitialLocations)//10
        {
            char Terrain;
            for (int x = 0; x < LandscapeSize; x++)
            {
                for (int y = 0; y < LandscapeSize; y++)//10
                {
                    if (x == 5 || y == 2)
                    {
                        Terrain = 'R';
        ...
        private void CreateNewWarren()//11
        {
            int x, y;
            do
            {
            ...
            } while (Landscape[x, y].Warren != null || Landscape[x, y].TypeOfTerrain == 'R' );
            if (ShowDetail)
            {
        ...
        private void CreateNewFox()//11
        {
            ...
            do
            {
                x = Rnd.Next(0, LandscapeSize);
                y = Rnd.Next(0, LandscapeSize);
            } while (Landscape[x, y].Fox != null || Landscape[x, y].TypeOfTerrain == 'R');
            if (ShowDetail)
            {
        ...
        private void FoxesEatRabbitsInWarren(int WarrenX, int WarrenY)//12
        {
            int FoodConsumed;
            int PercentToEat;
            double Dist;
            ...
            for (int FoxX = 0; FoxX < LandscapeSize; FoxX++)
            {
                ...
                {
                    if (CheckIfPathCrossesRiver(FoxX, FoxY, WarrenX, WarrenY) == false)//12
                    {
        ...
        private void DrawLandscape()//11
        {
            ...
            Console.WriteLine();
            for (int x = 0; x <= LandscapeSize * 5 + 3; x++)//11
            {
                Console.Write("-");
            }
            Console.WriteLine();
            ...
            for (int y = 0; y < LandscapeSize; y++)
            {
                ...
                for (int x = 0; x < LandscapeSize; x++)
                {
                    ...
                    Console.Write(Landscape[x, y].TypeOfTerrain);//11
                    Console.Write("|");
        ...
        private bool CheckIfPathCrossesRiver(int FoxX, int FoxY, int RabbitX, int RabbitY)//12
        {
            if (FoxX < RabbitX)
            {
                for (int i = FoxX; i < RabbitX; i++)//Left
                {
                    if (Landscape[i, FoxY].TypeOfTerrain == 'R')
                    {
                        return true;
                    }
                }
            ...
            {
                for (int i = FoxX; i > RabbitX; i--)//Right
                {
                    if (Landscape[i, FoxY].TypeOfTerrain == 'R')
                    {
                        return true;
            ...
            if(FoxY < RabbitY)
            {
                for (int i = FoxY; i < RabbitY; i++)//Down
                {
                    if (Landscape[FoxX, i].TypeOfTerrain == 'R')
                    {
                        return true;
                    }
                }
    ...
    class Rabbit : Animal
    {
        ...
        public override void CalculateNewAge()//10
        {
